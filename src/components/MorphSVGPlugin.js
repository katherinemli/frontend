...
let _DEG2RAD = Math.PI / 180
var _RAD2DEG = 180 / Math.PI
var _svgPathExp = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi
var _numbersExp = /(?:(-|-=|\+=)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi
var _selectorExp = /(^[#\.][a-z]|[a-y][a-z])/gi
var _commands = /[achlmqstvz]/gi
...
var TweenLite = _gsScope._gsDefine.globals.TweenLite
// _nonNumbersExp = /(?:([\-+](?!(\d|=)))|[^\d\-+=e]|(e(?![\-+][\d])))+/ig,

let _log = function(message) {
  if (_gsScope.console) {
    console.log(message)
  }
}
...
let _normalizedArcToBeziers = function(angleStart, angleExtent) {
  let segments = Math.ceil(Math.abs(angleExtent) / 90);
				var l = 0;
				var a = [];
				var angleIncrement; var controlLength; var angle; var dx; var dy; var i;
  angleStart *= _DEG2RAD
  angleExtent *= _DEG2RAD
  angleIncrement = angleExtent / segments
  controlLength =
    ((4 / 3) * Math.sin(angleIncrement / 2)) /
    (1 + Math.cos(angleIncrement / 2))
...
var _arcToBeziers = function(
  ...
  let bezierPoints = _normalizedArcToBeziers(angleStart, angleExtent);
				var a = cosAngle * rx;
				var b = sinAngle * rx;
				var c = sinAngle * -ry;
				var d = cosAngle * ry;
				var l = bezierPoints.length - 2;
				var i; var px; var py;
...
let _pathDataToBezier = function(d) {
  var a =
      (d + '')
        ...
        .match(_svgPathExp) || []; //some authoring programs spit out very small numbers in scientific notation like "1e-5", so make sure we round that down to 0 first.
				...
				var l = 2;
				var points = 0;
				var i; var j; var x; var y; var command; var isRelative; var segment; var startX; var startY; var difX; var difY; var beziers; var prevCommand;
...
let _subdivideBezier = function(bezier, quantity) {
  let tally = 0;
				var max = 0.999999;
				var l = bezier.length;
				var newPointsPerSegment = quantity / ((l - 2) / 6);
				var ax; var ay; var cp1x; var cp1y; var cp2x; var cp2y; var bx; var by;
				var x1; var y1; var x2; var y2; var i; var t;
  for (i = 2; i < l; i += 6) {
...
let _bezierToPathData = function(beziers) {
  var data = '';
				var l = beziers.length;
				var rnd = 100;
				var sl; var s; var i; var segment;
  for (s = 0; s < l; s++) {
    segment = beziers[s]
...
let _reverseBezier = function(bezier) {
  let a = [];
				var i = bezier.length - 1;
				var l = 0;
  while (--i > -1) {
    a[l++] = bezier[i]
    a[l++] = bezier[i + 1]
...
let _getAverageXY = function(bezier) {
  let l = bezier.length;
				var x = 0;
				var y = 0;
				var i;
...
var _getTotalSize = function(bezier) {
...
let _sortByComplexity = function(a, b) {
  return b.length - a.length
}
let _sortBySize = function(a, b) {
  let sizeA = a.size || _getSize(a);
				var sizeB = b.size || _getSize(b);
...
let _offsetBezier = function(bezier, shapeIndex) {
  let a = bezier.slice(0);
				var l = bezier.length;
				var wrap = l - 2;
				var i; var index;
...
let _getTotalMovement = function(sb, eb, shapeIndex, offsetX, offsetY) {
  let l = sb.length;
				var d = 0;
				var wrap = l - 2;
				var index; var i; var x; var y;
  shapeIndex *= 6
  for (i = 0; i < l; i += 6) {
    index = (i + shapeIndex) % wrap
...
var _getClosestShapeIndex = function(sb, eb, checkReverse) {
  ...
  let l = sb.length;
				var sCenter = _getAverageXY(sb); //when comparing distances, adjust the coordinates as if the shapes are centered with each other.
				var eCenter = _getAverageXY(eb);
				var offsetX = eCenter[0] - sCenter[0];
				var offsetY = eCenter[1] - sCenter[1];
				var min = _getTotalMovement(sb, eb, 0, offsetX, offsetY);
				var minIndex = 0;
				var copy; var d; var i;
  for (i = 6; i < l; i += 6) {
...
var _getClosestAnchor = function(bezier, x, y) {
  ...
  let j = bezier.length;
				...
				var closestX = 0;
				var closestY = 0;
				var b; var dx; var dy; var d; var i; var l;
  while (--j > -1) {
    b = bezier[j]
    l = b.length
    for (i = 0; i < l; i += 6) {
...
var _getClosestSegment = function(
  bezier,
  pool,
  startIndex,
  sortRatio,
  ...
  let l = pool.length;
				var index = 0;
				var minSize = Math.min(bezier.size || _getSize(bezier), pool[startIndex].size || _getSize(pool[startIndex])) * sortRatio; //limit things based on a percentage of the size of either the bezier or the next element in the array, whichever is smaller.
				var min = 999999999999;
				var cx = bezier.centerX + offsetX;
				var cy = bezier.centerY + offsetY;
				var size; var i; var dx; var dy; var d;
...
var _equalizeSegmentQuantity = function(start, end, shapeIndex, map) {
  // returns an array of shape indexes, 1 for each segment.
  let dif = end.length - start.length;
				var longer = dif > 0 ? end : start;
				var shorter = dif > 0 ? start : end;
				var added = 0;
				var sortMethod = (map === "complexity") ? _sortByComplexity : _sortBySize;
				var sortRatio = (map === "position") ? 0 : (typeof(map) === "number") ? map : 0.8;
				var i = shorter.length;
				var shapeIndices = (typeof(shapeIndex) === "object" && shapeIndex.push) ? shapeIndex.slice(0) : [shapeIndex];
				var reverse = (shapeIndices[0] === "reverse" || shapeIndices[0] < 0);
				var log = (shapeIndex === "log");
				var eb; var sb; var b; var x; var y; var offsetX; var offsetY;
  if (!shorter[0]) {
    return
  }
  if (longer.length > 1) {
...
let _pathFilter = function(a, shapeIndex, map, precompile) {
  let start = _pathDataToBezier(a[0]);
				var end = _pathDataToBezier(a[1]);
  if (
...
let _buildPathFilter = function(shapeIndex, map, precompile) {
  return map || precompile || shapeIndex || shapeIndex === 0
    ? function(a) {
        _pathFilter(a, shapeIndex, map, precompile)
      }
    : _pathFilter
}
let _offsetPoints = function(text, offset) {
  if (!offset) {
    return text
  }
  let a = text.match(_numbersExp) || [];
				var l = a.length;
				var s = "";
				var inc; var i; var j;
  if (offset === 'reverse') {
...
let _equalizePointQuantity = function(a, quantity) {
  let tally = 0;
				var x = parseFloat(a[0]);
				var y = parseFloat(a[1]);
				var s = x + "," + y + " ";
				var max = 0.999999;
				var newPointsPerSegment; var i; var l; var j; var factor; var nextX; var nextY;
  l = a.length
...
let _pointsFilter = function(a) {
  let startNums = a[0].match(_numbersExp) || [];
				var endNums = a[1].match(_numbersExp) || [];
				var dif = endNums.length - startNums.length;
  if (dif > 0) {
...
let _buildPointsFilter = function(shapeIndex) {
...
let _createPath = function(e, ignore) {
  var path = _gsScope.document.createElementNS(
      'http://www.w3.org/2000/svg',
      'path'
    );
				var attr = Array.prototype.slice.call(e.attributes);
				var i = attr.length;
				var name;
  ignore = ',' + ignore + ','
  while (--i > -1) {
...
let _convertToPath = function(e, swap) {
  let type = e.tagName.toLowerCase();
				var circ = 0.552284749831;
				var data; var x; var y; var r; var ry; var path; var rcirc; var rycirc; var points; var w; var h; var x2; var x3; var x4; var x5; var x6; var y2; var y3; var y4; var y5; var y6;
  ...
  if (type === 'rect') {
    ...
    ) // previously, we just converted to "Mx,y Lx,y" but Safari has bugs that cause that not to render properly when using a stroke-dasharray that's not fully visible! Using a cubic bezier fixes that issue.
...
let _parseShape = function(shape, forcePath, target) {
...
var MorphSVGPlugin = _gsScope._gsDefine.plugin({
  ...
  init: function(target, value, tween, index) {
    ...
    if (type !== 'PATH' && !isPoly) {
      _log(
        'WARNING: cannot morph a <' + type + '> SVG element. ' + _morphMessage
      )
      return false
    }
    ...
    if (shape) {
      ...
      pt = this._addTween(
        ...
        false,
        p,
        typeof value.precompile === 'object'
          ? function(a) {
          ...
          ? _buildPathFilter(
              value.shapeIndex,
              value.map || MorphSVGPlugin.defaultMap,
              value.precompile
            )
          : _buildPointsFilter(value.shapeIndex)
      )
      if (pt) {
